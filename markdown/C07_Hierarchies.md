...
为了将关于子群的引理应用于子群或子环，我们需要一个类，就像变换一样。请注意，此类接受一个`SetLike`实例作为参数，所以它不需要一个carrier字段，并可以在其字段中使用成员标记。
...
作为练习，你应该定义一个 `Subgroup₁` 结构，为其赋予一个 `SetLike` 实例和一个 `SubmonoidClass₁` 实例，将一个 `Group` 实例放在与 `Subgroup₁` 关联的子类型上，并定义一个 `SubgroupClass₁` 类。
...
我们这一章的最后一个主题是quotients。我们再次要解释在 Mathlib 中如何构建方便的表示法，并避免代码重复。这里主要的设备是 `HasQuotient` 类，它允许像 `M ⧸ N` 这样的记号。注意，商数符号 `⧸` 是一个特殊的 Unicode 字符，不是一个常规的 ASCII 除号符号。
